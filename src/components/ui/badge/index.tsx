...
// The icon is typed loosely so NativeWind v4's generic options type stops
// narrowing `nativeStyleToProp` to this component's own prop names. gluestack
// authors these components against NativeWind v5, which accepts the object as
// written; v4 runs it identically but types it more tightly.
cssInterop(UIIcon as React.ComponentType<Record<string, unknown>>, {
  className: {
    target: 'style',
    nativeStyleToProp: {
      height: true,
...
>(function BadgeIcon({ className, size, ...props }, ref) {
  const { variant: parentVariant } = useStyleContext(SCOPE);
  // `UIIcon` accepts a union of icon shapes, and `forwardRef` cannot describe a
  // single instance type that satisfies every member of it. The ref is handed
  // on untouched; only its type is restated.
  const iconRef = ref as React.ComponentProps<typeof UIIcon>['ref'];
